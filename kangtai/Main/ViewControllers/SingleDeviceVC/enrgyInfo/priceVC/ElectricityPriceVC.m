...
- (void)initUI
{
    ...
    energyUnitLabel.font = [UIFont systemFontOfSize:18];
    ...
    costTextField.textColor = RGBA(30.0, 180.0, 240.0, 1.0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == [currencyArr indexOfObject:currencyString]) {
            [currencyBtn setTitleColor:RGBA(0, 150, 230, 1) forState:UIControlStateNormal];
        } else {
...
- (void)changeCurrency:(UIButton *)btn
{
    for (int i = 0; i < 5; i++) {
        ...
        if (i == btn.tag - 100) {
            [tempBtn setTitleColor:RGBA(0, 150, 230, 1) forState:UIControlStateNormal];
